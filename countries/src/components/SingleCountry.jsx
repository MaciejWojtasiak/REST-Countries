            ...
            <div className="country_details">
              ...
              <div className="country_props">
                ...
                <div className="col">
                  ...
                  <p className="country_languages">Languages: <span className="value">{languages ? languages.map((language, index) => (<span key={index}>{index > 0 ? ", ": ''}{language.name}</span>)) : ""}</span></p>  
                </div>
               
              </div>
              <div className="borders-container">
                <p className="country_borders">Border Countries: </p><div className="links">{bordersNames.length != 0 ? bordersNames.map((border, index)=>(<span className="value" key={index}><Link className="link border-link" to={`/country/${border.alpha3Code}`}>{border.name}</Link></span>)): "No borders."}</div>
              </div>
